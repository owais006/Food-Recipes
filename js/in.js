...
async function meal() {
    ...
    // check(data);
    // search(image,area,cat,name,count,data,id,inst,video);
    }
}
}
...
// function search(){
//     searchData=document.getElementById("s_data").value;
//     console.log(searchData);
//     for(i=0;i<data.length;i++){
//         if(data[i].strMeal.includes(searchData)){
//         const image=data[i].strMealThumb;
//         const id = data[i].idMeal;
//         const area=data[i].strArea;
//         const cat=data[i].strCategory; 
...
//             create_card(image,area,cat,name,count,data,id,inst,video);
//         }
//         // ele.data.toggle("hide",!isvisible)
